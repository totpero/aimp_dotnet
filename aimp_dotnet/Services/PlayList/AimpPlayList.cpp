...
namespace AIMP
{
	namespace SDK
	{
		namespace PlayList
		{
			...
			{
				IAIMPPlaylist *playList = (IAIMPPlaylist*)AIMP::SDK::ManagedAimpCore::QueryInterface(IID_IAIMPPlaylist);
				GetPropertyList();
			}
			...
			{
				IAIMPString *url = ObjectHelper::MakeAimpString(AIMP::SDK::ManagedAimpCore::GetAimpCore(), fileUrl);
			...
			AimpActionResult AimpPlayList::AddList(System::Collections::Generic::IList<IAimpFileInfo^>^ fileUrlList, PlayListFlags flags, PlayListFilePosition filePosition)
			{
				if (fileUrlList->Count > 0)
				{
					...
					AIMP::SDK::ManagedAimpCore::GetAimpCore()->CreateObject(IID_IAIMPObjectList, (void**)&list);
			...
			AimpActionResult AimpPlayList::AddList(IList<String^>^ fileUrlList, PlayListFlags flags, PlayListFilePosition filePosition)
			{
				if (fileUrlList->Count > 0)
				{
					...
					{
						list->Add(ObjectHelper::MakeAimpString(AIMP::SDK::ManagedAimpCore::GetAimpCore(), file));
					}
